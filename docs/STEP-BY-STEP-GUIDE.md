...
| Notebook 03 export prints `Failed to convert the model to the target version 17` and quantize crashes with `shape differ` | Expected on torch 2.11 — the exporter now writes opset 18 and the old dynamic-batch export confused the quantizer. Re-run Cell 1 (git pull → fixed `inference/export.py`), set `winner` to exactly one name, then re-run Cell 3 (export/quantize/verify). |
