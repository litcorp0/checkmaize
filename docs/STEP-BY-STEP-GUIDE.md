...
| Notebook 03 export prints `Failed to convert the model to the target version 17` or quantize crashes with `Inferred shape and existing shape differ` | torch 2.11 exports opset 18 and its graphs confuse the quantizer's strict shape check. Re-run Cell 1 (git pull → fixed `inference/export.py` + `inference/quantize.py`), confirm `winner` is exactly one name, then re-run Cell 3. |
